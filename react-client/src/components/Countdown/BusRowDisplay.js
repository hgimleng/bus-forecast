...
function BusRowDisplay({busNum, arrival1, arrival2, arrival3, currentTime, stopLatLon, onBusRowClick, settings, getDirection}) {
    return (
        <tr>
            <td className="align-middle text-center"><button className='btn btn-outline-primary' onClick={() => onBusRowClick(busNum, arrival1['destination_code'])}>{ busNum }</button></td>
            <BusArrivalCellDisplay time={arrival1['time']} type={arrival1['type']} load={arrival1['load']} currentTime={currentTime} stopLatLon={stopLatLon} busLatLon={[arrival1['lat'], arrival1['lng']]} settings={settings} getDirection={getDirection} />
